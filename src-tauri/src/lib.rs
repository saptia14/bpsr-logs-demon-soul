...
pub fn run() {
    ...
    let builder = Builder::<tauri::Wry>::new()
        ...
        .commands(collect_commands![
            ...
            live::commands::set_webhook_enabled,
    ...
    let tauri_builder = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // The Discord webhook is privacy-sensitive (it uploads party names,
            // classes, scores and a screenshot). Never enable it from the
            // persisted store here: at this point the frontend Svelte store has
            // not loaded from disk yet, so `get_or` can return a stale value
            // and fire a report the UI shows as disabled. We always start
            // disabled and let the frontend push the real value once its
            // settings store has loaded (see src/routes/+layout.svelte).
            app.manage(create_webhook_enabled(false)); // setup webhook enabled state (off until frontend syncs)
